...
!     character*80, dimension(:), pointer :: temp
     ...
     case (READ_WRITE)
        if (write_model .and. write_data) then
        	...
        	call write_Cond2D(fidWrite,cUserDef%wFile_Model,sigma0,grid)
     ...
     case (COMPUTE_J)
        ...
        call writeAll_Cond2D(fidWrite,cUserDef%wFile_Sens,   &
     ...
     case (MULT_BY_J_T)
        ...
        call write_Cond2D(fidWrite,cUserDef%wFile_dModel,dsigma,grid)
     ...
     case (MULT_BY_J_T_MTX)
        ...
        call writeAll_Cond2D(fidWrite,cUserDef%wFile_dModelMTX,   &
                        allData%nTx,sigma,'J^T x d (all transmitters)')

     case (INVERSE_NLCG)
        write(*,*) 'Starting the NLCG search...'
        call NLCGsolver(allData,cUserDef%lambda,sigma0,sigma1,cUserDef%alpha)
        call write_Cond2D(fidWrite,cUserDef%wFile_Model,sigma1,grid)
        if (write_data) then
